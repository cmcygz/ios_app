...
- (id)initWithItems:(NSArray *)items {
	self = [super initWithFrame:CGRectMake(0.0, 520.0, 320.0, 49.0)];
    // TODO:
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor colorWithRed:0.565 green:0.741 blue:0.102 alpha:1];
		self.delegate = self;
		...
		for (double d = 0; d < ceil(items.count / 5.0); d ++) {
            ...
            tabBar.barTintColor = [UIColor colorWithRed:0.549 green:0.737 blue:0.278 alpha:1];
...
- (void)setItems:(NSArray *)items animated:(BOOL)animated {
	for (UITabBar *tabBar in self.tabBars) {
		int len = 0;
		for (long i = [self.tabBars indexOfObject:tabBar] * 5; i < [self.tabBars indexOfObject:tabBar] * 5 + 5; i ++)
			if (i < items.count)
				len ++;
...
- (void)tabBar:(UITabBar *)cTabBar didSelectItem:(UITabBarItem *)item {
    ...
    NSLog(@"Selected %ld",(long)item.tag);
}
